...
The medical imaging extension proposal represents imaging characteristics through two new tables—Image_occurrence and Image_feature.
...
|image_occurrence|local_path|No|varchar (max)|"Universal Naming Convention (UNC) path to the folder containing the image object file access via a storage block access protocol. (e.g., \\Server\Directory)"|
...
|image_occurrence|modality_concept_id|Yes|integer|The concept_id of the OMOP standard concept corresponding to the DICOM-defined modality value (e.g., US, CT, MR, PT, DR, CR, NM).|
...
|image_feature|image_feature_event_field_concept_id|No|integer|The concept_id of the domain table in which the feature is stored (e.g., Measurement, Observation). This concept should be used together with the image_feature_event_id.|
|image_feature|image_feature_event_id|No|integer|The primary key id of the record in the domain table in which the feature is stored.|
